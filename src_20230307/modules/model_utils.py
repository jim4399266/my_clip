...
def get_text_backbone(config, backbone_config_path=None, pretrained=True):
    ...
    # 加载预训练模型
    # backbone_config.add_cross_attention = True
    if pretrained:
...
def get_image_backbone(config):
    ...
    # 两个模态的编码器，使用预训练好的模型
    if is_clip:
        image_backbone, vision_width = build_model(config['vit'], resolution_after=config['image_size'])
    return image_backbone, vision_width
