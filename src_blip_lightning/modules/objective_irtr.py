...
def val_irtr(pl_module, data_loader):
    ...
    if pl_module.trainer.world_size > 1:
        # pl_module.trainer.strategy.barrier()
        # pl_module.trainer.strategy.reduce(score_matrix_i2t, op=torch.distributed.ReduceOp.SUM)
        # pl_module.trainer.strategy.reduce(score_matrix_t2i, op=torch.distributed.ReduceOp.SUM)
