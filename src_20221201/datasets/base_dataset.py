...
class CocoKarpathyBaseDataset(Dataset):
    ...
    def __getitem__(self, item):
        raise NotImplementedError("return a function to get items in specific dataset")
    ...
    def get_image(self, image_index, image_key='image'):
        ...
        return {
            ...
            # 'img_index': self.index_mapper[raw_index][0],
            # 'cap_index': self.index_mapper[raw_index][1],
            # 'raw_index': raw_index,
        }
    ...
    def get_suite(self, raw_index):
        # 获取一组训练数据
        ret = dict()
        try:
            ret.update(self.get_image(raw_index))
            if not self.image_only:
                text = self.get_text(raw_index)
                # ret.update({'replica': True if text['cap_index'] > 0 else False})
                ret.update(text)
        except Exception as e:
            print(f"Error while read file idx {raw_index} in {self.names[0]} -> {e}")
        return ret
