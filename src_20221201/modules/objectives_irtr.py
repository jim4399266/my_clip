...
def irtr_encoder(pl_module, dataloader):
    ...
    ret = dict()
    tokenizer = pl_module.trainer.datamodule.dm_list[0].tokenizer
    max_len = pl_module.trainer.datamodule.dm_list[0].max_text_len
    for batch in tqdm(dataloader, ncols=80, desc='Getting embedding...'):
        text_list = [text for texts in batch['text_list'] for text in texts]
        ...
        batch['text_encodings'] = {k: v.to(device) for k, v in encodings.items()}
...
def trans_dot_recall(pl_module, encoder_ret):
    ...
    # 计算所有图片和所有文本的相似度
    image_logits, text_logits = pl_module.itc_logits(image_cls, text_cls)
