...
def compute_irtr(pl_module, encoder_ret, phase):
    ...
    logits_per_image, logits_per_text = pl_module.itc_logits(encoder_ret['image_cls'], encoder_ret['text_cls'])
...
def compute_irtr_recall(pl_module, phase):
    # 加载数据集
    if phase == 'val':
        dataloader = pl_module.trainer.datamodule.dm_list[0].val_dataloader()
    else:
        dataloader = pl_module.trainer.datamodule.dm_list[0].test_dataloader()
