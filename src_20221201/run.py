...
def main(_config):
    ...
    dm = MTDataModule(config, dist=dist)
    # dm.prepare_data()
    # dm.setup(stage='fit')
    # for epoch in range(3):
    #     for i, batch in enumerate(dm.val_dataloader()):
    #         print(batch['text_index'],end=' ')
    #     print('--')
    ...
    if config['load_path'] == "":
        ...
        log_name = f'bs{config["batch_size"]}_pbs{config["per_gpu_batchsize"]}_epoch{config["max_epoch"]}_lr{config["learning_rate"]}_is{config["image_size"]}_from_{config["load_path"].split("/")[-1][:-5]}'
        saved_dir = Path(output_dir) / f'bs{config["batch_size"]}_pbs{config["per_gpu_batchsize"]}_epoch{config["max_epoch"]}_lr{config["learning_rate"]}_is{config["image_size"]}_from_{config["load_path"].split("/")[-1][:-5]}'
