...
class ITCHead(nn.Module):
    ...
    def __init__(self, logit_scale: Union[int, float, nn.Parameter] = None):
        ...
        self.logit_scale = nn.Parameter(torch.tensor(0.07) * torch.ones([]))
        # self.logit_scale = nn.Parameter(0.07 * torch.ones([]))
        # self.logit_scale = 0.07
        # self.logit_scale = 20

    def clamp_scaler(self, min=0.001, max=0.5):
        with torch.no_grad():
            # self.logit_scale = torch.clamp(self.logit_scale, min, max)
            # self.logit_scale = self.logit_scale.clamp(0.001, 0.5)
            self.logit_scale.clamp_(min, max)

    def forward(self, features_A, features_B, normalized=False, scaling=True):
        if not normalized:
        ...
        logits_A = features_A @ features_B.t()
        if scaling:
            logits_A = logits_A / self.logit_scale
        return logits_A