...
class ModelModule(pl.LightningModule):
    def __init__(self, config):
        ...
        self.text_backbone = model_utils.get_text_backbone(config)
        self.image_backbone, self.vision_width = model_utils.get_image_backbone(config)

        self.token_type_embeddings = nn.Embedding(2, config['hidden_size'])
        self.token_type_embeddings.apply(objectives.init_weights)

        self.cross_modal_text_transform = nn.Linear(self.text_backbone.config.hidden_size, config['hidden_size'])
        self.cross_modal_text_transform.apply(objectives.init_weights)
        self.cross_modal_image_transform = nn.Linear(self.vision_width, config['hidden_size'])
        ...
        self.text_backbone_m = model_utils.get_text_backbone(config)
        self.image_backbone_m, self.vision_width = model_utils.get_image_backbone(config)

        self.token_type_embeddings_m = nn.Embedding(2, config['hidden_size'])
        self.token_type_embeddings_m.apply(objectives.init_weights)

        self.cross_modal_text_transform_m = nn.Linear(self.text_backbone.config.hidden_size, config['hidden_size'])
        self.cross_modal_text_transform_m.apply(objectives.init_weights)
        self.cross_modal_image_transform_m = nn.Linear(self.vision_width, config['hidden_size'])
        self.cross_modal_image_transform_m.apply(objectives.init_weights)

        self.cross_modal_image_pooler_m = heads.Pooler(config['hidden_size'])
        self.cross_modal_image_pooler_m.apply(objectives.init_weights)
        self.cross_modal_text_pooler_m = heads.Pooler(config['hidden_size'])
        self.cross_modal_text_pooler_m.apply(objectives.init_weights)

        self.model_pairs = [
            [self.text_backbone, self.text_backbone_m],
            [self.image_backbone, self.image_backbone_m],
            [self.token_type_embeddings, self.token_type_embeddings_m],
            [self.cross_modal_text_transform, self.cross_modal_text_transform_m],
            [self.cross_modal_image_transform, self.cross_modal_image_transform_m],
            [self.cross_modal_text_pooler, self.cross_modal_text_pooler_m],
            [self.cross_modal_image_pooler, self.cross_modal_image_pooler_m]
        ]
    ...
    #
    #     model_utils.freeze_module(self.image_backbone.embeddings)
    ...
    def text_encoder_m(self, batch, mask_text=False):
        text_encodings = batch.get(f"text_encodings", None)
        input_ids = text_encodings.get("input_ids", None)
        text_masks = text_encodings.get("attention_mask", None)
        # 文本编码
        text_embeds_m = self.text_backbone_m(input_ids=input_ids, attention_mask=text_masks)[0]
        text_embeds_m = self.cross_modal_text_transform_m(text_embeds_m)
        text_extend_masks_m = self.text_backbone_m.get_extended_attention_mask(
            text_masks, text_masks.size(), device=self.device)
        text_embeds_m = text_embeds_m + self.token_type_embeddings_m(torch.zeros_like(text_masks))
        text_cls_m = self.cross_modal_text_pooler_m(text_embeds_m)
        # text_cls_m_all = torch.cat([text_cls_m.T, self.text_queue.clone().detach()], dim=0)
        return {
            'text_embeds_m': text_embeds_m,
            'text_extend_masks_m': text_extend_masks_m,
            'text_cls_m': text_cls_m,
            # 'text_cls_m_all': text_cls_m_all,
        }
    ...
    def image_encoder_m(self, batch, image_token_type_idx=1):
        img = batch.get('image', None)
        iids = batch.get('image_index', None)
        # 图像编码
        # vit 将图片分成patch，比如图片大小是224，patch大小 32
        # 则一共分成 (224 / 32)^2 + 1 个patch，多出来的1是cls标志
        image_embeds_m = self.image_backbone_m(img)
        image_embeds_m = self.cross_modal_image_transform(image_embeds_m)

        image_masks_m = torch.ones((image_embeds_m.size(0), image_embeds_m.size(1)), dtype=torch.long, device=self.device)
        image_extend_masks_m = self.text_backbone_m.get_extended_attention_mask(image_masks_m, image_masks_m.size(), device=self.device)
        # TODO 作者根据ViLT的方法，加上 token_type_embedding
        image_embeds_m = image_embeds_m + self.token_type_embeddings_m(torch.full_like(image_masks_m, image_token_type_idx))
        image_cls_m = self.cross_modal_image_pooler(image_embeds_m)
        # image_cls_m_all = torch.cat([image_cls_m.T, self.image_queue.clone().detach()], dim=0)
        return {
            'image_embeds_m': image_embeds_m,
            'image_masks_m': image_masks_m,
            'image_extend_masks_m': image_extend_masks_m,
            'image_cls_m': image_cls_m,
            # 'image_cls_m_all': image_cls_m_all,
            'iids': iids.long(),
        }
    ...
    def forward(self, batch, phase):
        ...
        with torch.no_grad():
            ...
            encoder_ret.update(self.text_encoder_m(batch))
            encoder_ret.update(self.image_encoder_m(batch))

        # # Image Retrieval and Text Retrieval
        if "irtr" in self.current_tasks:
            # ret.update(objectives.compute_irtr(self, encoder_ret, phase))
            ret.update(objectives.compute_irtr_m(self, encoder_ret, phase))
    ...
    def training_step(self, batch, batch_idx):
        ...
        # momentum = True
        # if momentum:
        if batch_idx % self.hparams.config['grad_steps'] == 0:
            with torch.no_grad():
                self._momentum_update()
